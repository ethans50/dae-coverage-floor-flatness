...
지표는 전부 `analyze_coverage_comparison.py`가 저장된 산출물에서 계산됨:
...
| **순주행시간(Active Time)** | 총 시간에서 Stall 시간을 뺀 값. 경로 planning의 효과를 Stall 발생 여부와 분리해 비교하기 위함. |
| **효율(Efficiency)** | 단위 거리당 / 단위 시간당 Completeness 증가율. |

## 실험 조합

5개 최적화에 대한 leave-one-out ablation에 Anchor 조합 2개를 더한 구성:
...
각 조합은 *n* ≥ 4회 반복함. 매 주행을 `eval_runs/<라벨>/`로 스냅샷해두면 조합끼리 서로 덮어쓰지 않음.
...
- **planning-실행 파라미터 대조.** `final_path_meta.json`에 planning 시점 값을 기록하고, `mission_executor`가 현재 설정과 다르면 시작을 거부함.
