...
바닥 평탄도는 마감 공정의 계약상 인수 기준임. 현장에서 쓰이는 방법은 두 가지임:

- **라인레이저 프로파일로미터(Line-Laser Profilometer)** — 정밀도는 실험실급이지만 스마트팩토리 가격대라 일반 주택 현장에는 도입되지 않음.
- **수동 레이저 레벨기** — 최근 부실시공 이슈로 빠르게 늘어난 방식. 저렴하지만 사람이 방마다 장비를 들고 이동해야 해서 **인력과 시간이 세대 수에 비례해 선형으로 증가**함.
...
평면이 반복된다는 점이 핵심임. 비용이 큰 계획 단계를 **설계 파일로부터 오프라인에서 한 번만** 수행하고, 같은 평면의 모든 세대에 재사용함.
...
- **Drives autonomously.** Nav2 + AMCL을 쓰되, 측정(Coverage) 구간과 이동(Transit) 구간에 서로 다른 Controller를 적용함.
...
| 역할 | 기기 | 실행 | 온라인 여부 |
|---|---|---|---|
| **계획** | 워크스테이션/노트북 (x86_64) | `run_generation_pipeline.py` | 오프라인, 평면당 1회 |
...
산출물: `final_topological_map.npz` — 노드 마스크, 중심점, 그리고 노드 간 연결 지점(문).
...
3. **국소 재정렬.** 허브 복도에 연결된 말단 방(Pendant)들을, 로봇이 허브 Coverage를 실제로 끝내는 지점에서 문이 가까운 순서로 다시 배열함.
4. **노드 내부 Coverage.** Fields2Cover가 Boustrophedon Swath를 생성함. wide 노드는 최적 각도를 자동 탐색하고, narrow 노드는 각도를 고정함.
...
7. **내보내기.** 픽셀 좌표를 미터로 변환하고 일정 간격으로 Resampling해 `final_path.json`으로 저장함. 계획 시점에 실제로 쓴 파라미터를 기록한 `final_path_meta.json`도 함께 남김.
...
**시작 시**
1. `final_path.json`을 읽고 `final_path_meta.json`을 현재 `params.yaml`과 대조함. 계획된 경로의 좌표는 계획 시점 파라미터에 기하학적으로 의존하므로, 값이 다르면 시작하지 않음.
...
<p align="center">
  ...
  <em>계획 Waypoint와 AMCL 기준 실주행 Trajectory. 매 주행이 끝날 때 자동 생성됨.</em>
</p>
...
- **구간별 Controller 분리.** 측정 구간은 Swath를 일정한 속도로 반복 가능하게 추종해야 하고, 이동 구간은 문과 코너를 연속적인 감가속으로 통과해야 함. 한 Controller로 두 요구를 함께 맞추기는 불리하므로, 측정은 RotationShim + DWB, 이동은 계획 경로를 직접 추종하는 Regulated Pure Pursuit(RPP)로 나눔. Transit BT의 `RemovePassedGoals` 반경을 0.3 m로 줄인 것도 같은 맥락임 — 문 앞 Via-point가 도달 전에 제거되지 않아야 계획한 통과 형상이 유지됨.
...
- **계획 파라미터 고정.** 계획과 실행이 서로 다른 기기에서 돌아가는 구조에서는 두 기기의 설정이 어긋나도 경로 좌표만으로는 알아채기 어려움. `final_path_meta.json`에 계획 시점 값을 기록하고, `mission_executor`가 시작 시 현재 설정과 대조함.

### 주행 중 실패 처리

오프라인 계획 경로를 반응형 Local Planner로 추종하는 구조에서는, 계획 단계와 Nav2 어느 한쪽만으로는 해소되지 않는 Stall(정체)이 생길 수 있음. `mission_executor`가 이를 직접 감지하고 단계적으로 대응함:

- **Stall Watchdog.** Nav2 Recovery가 진전 없이 반복되는 경우에 대비해, `mission_executor`가 진행 상황을 별도로 추적하고 `nav_stuck_cancel_sec` 동안 진전이 없으면 Action을 취소함.
- **3단계 Escalation.** 취소 후 (1) Nav2 Action 재시도, (2) 몇 cm 후진 후 재시도 — 벽에 붙은 자세에서는 `Spin`의 사전 충돌 검사가 제자리 회전을 거부할 수 있기 때문, (3) `/cmd_vel`을 직접 발행해 짧고 느린, 거리 제한이 걸린 이동을 수행하되 AMCL 점프 감지와 하드 타임아웃으로 보호함. 단일 목표 주행은 2단계까지만 적용함.
...
pip install -r requirements-mission_generation.txt   # 경로 생성 기기
pip install -r requirements-surface_profiling.txt    # 측정 기기
```
...
```bash
# 0. [계획 기기] 경로 생성 후, ~/dae_floor_maps 를 로봇(Jetson)의 같은 위치로 복사
python3 run_generation_pipeline.py
...
모든 설정은 [`config/params.yaml`](config/params.yaml)에 파이프라인 단계별로 묶여 있음. 자주 수정하는 파라미터와 잘못 넣기 쉬운 값은 [설정 가이드](docs/kr/configuration.md)에 정리함.

## 평가

5개 최적화 요소 각각의 기여도를 leave-one-out ablation(9개 조합, 조합당 *n* ≥ 4회)으로 분리 측정함. 지표 정의, 실험 조합, 재현성 장치는 [평가 프로토콜](docs/kr/evaluation.md)에 정리함.

> **결과 수치는 아직 공개하지 않음.** 전체 매트릭스를 수집한 뒤 업로드 예정.
...
전체 파이프라인은 실물 하드웨어 없이 Gazebo 시뮬레이터에서도 그대로 동작함. ablation 실험은 이 방식으로 수행 및 측정함.
...
논문 준비 중임. 게재 전까지는 소프트웨어를 인용해 주기 바람:
...
현재 이 저장소에는 오픈소스 라이선스를 부여하지 않음. 저작권자의 사전 서면 허락 없이 코드와 문서를 복제·수정·배포할 수 없음. 라이선스는 논문 게재 이후 정할 예정임.

단, 이 패키지에 포함된 TurtleBot3 launch/description/Gazebo 파일은 원본 저작권자(ROBOTIS, Open Source Robotics Foundation)의 Apache-2.0 라이선스를 따르며, 각 파일의 원본 헤더를 그대로 유지함.
